(function ($) {
  ...
  $(document).ready(function () {
    function normalizeBrandAssets() {
      var brandLogoPath = "assets/img/logo/vk-interior-studio-logo.png";
      var brandAlt = "VK Interior Studio - Interior Design";

      $("#VK\\ Interior\\ Studio-smooth-wrapper").attr(
        "id",
        "antra-smooth-wrapper",
      );
      $("#VK\\ Interior\\ Studio-smooth-content").attr(
        "id",
        "antra-smooth-content",
      );

      $("[class*=\"VK Interior Studio-hover-view\"]").each(function () {
        this.className = this.className.replace(
          /VK Interior Studio-hover-view/g,
          "antra-hover-view",
        );
      });

      $(
        "img[src*=\"assets/img/logo/logo-1.png\"], img[src*=\"assets/img/logo/logo-2.png\"]",
      ).attr({
        src: brandLogoPath,
        alt: brandAlt,
      });
      ...
      $(".side-menu-logo img").css({
        maxWidth: "55px",
        width: "100%",
        height: "auto",
      });

      $(".mobile-side-menu .side-menu-head img").css({
        maxWidth: "55px",
        width: "100%",
        height: "auto",
      });
    ...
    function restoreTemplateImagePathsRuntime() {
      function restorePathValue(value) {
        if (!value) return value;
        if (value.indexOf("assets/img/") === -1) return value;
        if (value.indexOf("REPLACE-ME-") === -1) return value;
        return value.replace("REPLACE-ME-", "");
      }

      $("img[src]").each(function () {
        var current = $(this).attr("src");
        var restored = restorePathValue(current);
        if (restored !== current) {
          $(this).attr("src", restored);
        }
      });
      ...
      $("a[href]").each(function () {
        ...
        var restored = restorePathValue(current);
        if (restored !== current) {
          $(this).attr("href", restored);
        }
      });

      $("[data-background]").each(function () {
        var current = $(this).attr("data-background");
        var restored = restorePathValue(current);
        if (restored !== current) {
          $(this).attr("data-background", restored);
        }
      });

      $("[data-img]").each(function () {
        var current = $(this).attr("data-img");
        var restored = restorePathValue(current);
        if (restored !== current) {
          $(this).attr("data-img", restored);
        }
      });
    }

    normalizeBrandAssets();
    restoreTemplateImagePathsRuntime();
